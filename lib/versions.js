...
var doVersions = function () {
    ...
    return {
        ...
        fetchVersionsAsync: function (pipelines, done) {
            ...
            async.each(pipelines, function (currentPipeline, callback) {
                    if (pipeline.paused) {
                        //avoid callback overflow with large number of pipelines
                        return async.setImmediate(function () {
                            currentPipeline.skipVersion = true;
                            if (config.debug) {
                                console.log("skipping version on " + currentPipeline.name);
                            }
                            callback();
                        });
                        ...
                        //avoid callback overflow with large number of pipelines
                        return async.setImmediate(function () {
                            that.handleSameVersion(currentPipeline);
                            currentPipeline.skipVersion = true;
                            if (config.debug) {
                                console.log("skipping version on " + currentPipeline.name);
                            }
